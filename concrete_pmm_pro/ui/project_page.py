...
.cpmm-dashboard-card.info { border-left-color: #2563eb; }
...
def _dashboard_card_html(card: DashboardCard) -> str:
    ...
    return (
        f'<div class="cpmm-dashboard-card {status}">'
        f'<div class="cpmm-card-title">{escape(card.title)}</div>'
        f'<div class="cpmm-card-value">{escape(card.value)}</div>'
        f"{detail_html}"
        f'<span class="cpmm-status-badge {status}">{escape(status.upper())}</span>'
        "</div>"
    )
...
def _project_overview_cards(
    ...
    return [
        DashboardCard(
            ...
            "Generated section available" if section_geometry is not None else "Build a section before analysis",
        ...
        DashboardCard("Version", project.version, "Project metadata", "info"),
    ]
...
def _pre_report_readiness_cards(snapshot: Any, readiness: Any) -> list[DashboardCard]:
    ...
    return [
        DashboardCard("Overall Status", readiness.overall_status, "Pre-report readiness result", status_style_for_value(readiness.overall_status)),
        DashboardCard("ULS PMM Result", "Yes" if snapshot.pmm_result_available else "No", "Stored PMM result available", status_style_for_value("Yes" if snapshot.pmm_result_available else "No")),
        DashboardCard("SLS Result", "Yes" if snapshot.sls_result_available else "No", "Stored serviceability result available", status_style_for_value("Yes" if snapshot.sls_result_available else "No")),
        DashboardCard("Warning Count", f"{snapshot.warning_count:,}", "Stored engineering warnings", warning_status),
        DashboardCard(
...
def _report_foundation_cards(manifest: Any, snapshot: Any) -> list[DashboardCard]:
    ...
    return [
        DashboardCard("Report Manifest", "Yes" if manifest is not None else "No", "Report registry built", status_style_for_value("Yes" if manifest is not None else "No")),
        DashboardCard("Available Tables", f"{tables:,}", "Manifest table registry", "ready" if tables else "neutral"),
        DashboardCard("Available Figures", f"{figures:,}", "Manifest figure registry", "ready" if figures else "neutral"),
        DashboardCard("Engineering Limitations", f"{limitations:,}", "Report limitation registry", "warning" if limitations else "ready"),
    ]
...
def render_project_page() -> None:
    ...
    _render_dashboard_section("Analysis Configuration", _analysis_configuration_cards(analysis_mode), columns=5)

    _render_dashboard_section("SLS Stress Points", _sls_stress_point_cards(custom_points, include_default_stress_points), columns=3)
    ...
    _render_dashboard_section("Report Foundation", _report_foundation_cards(manifest, snapshot), columns=4)
